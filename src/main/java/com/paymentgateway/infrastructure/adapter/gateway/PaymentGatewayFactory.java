...
public class PaymentGatewayFactory {
     ...
     */
private PaymentGatewayPort selectOptimalGateway(List<PaymentGatewayPort> compatibleGateways, PaymentMethod paymentMethod) {
return switch(paymentMethod){
    case DEBIT_CARD, CREDIT_CARD, APPLE_PAY, GOOGLE_PAY -> compatibleGateways.stream()
        .filter(gateway -> "STRIPE".equals(gateway.getGatewayProvider()))
        .findFirst()
       .orElse(compatibleGateways.get(0));
    case PAYPAL -> compatibleGateways.stream()
        .filter(gateway -> "PAYPAL".equals(gateway.getGatewayProvider()))
        .findFirst()
        .orElse(compatibleGateways.get(0));
